...
import com.mygdx.PJs.Brick_Strong;
 ...
 */
public class Screen_lvl2 extends ScreenAdapter {
    ...
    public ArrayList<Brick_Strong> brick_strongs;
    ...
    public Screen_lvl2(BB_Game game) {
        this.game = game;
        time = 0;
        lastDelta = 0;
        ...
        this.brick_strongs = new ArrayList();
        ...
        for(int i = 0; i < 6; i++){
            ...
            for(int j = 1; j < 8; j = j + 2){
                ...
                this.bricks.add(new Brick ( 90 + (ANCHO * i), 900 + (ALTO * j)));
                bricksBreaked++;
            }
        }
        ...
        for(int i = 0; i < 3; i++){
            ...
            for(int j = 2; j < 7; j = j +2){
                ...
                this.brick_strongs.add(new Brick_Strong ( 180+ (ANCHO * 2f * i), 900 + (ALTO * j)));
            }
        }
    ...
    private void brickStrongDraw(){
        for (Brick_Strong brick_strong: this.brick_strongs) {
            game.batch.draw(Assets.brickStrong, brick_strong.position.x, brick_strong.position.y);
        }
    }
    ...
    private void brickStrongAct(){

        for (int i = 0; i <  this.brick_strongs.size(); i++){
            brick_strongs.get(i).update();

            if (pelota.bounds.overlaps(brick_strongs.get(i).bounds)) {
                pelota.brickColision(brick_strongs.get(i).position.x, brick_strongs.get(i).position.y, pelota.position.x, pelota.position.y, pelota.velocity.x, pelota.velocity.y);
                if(pelota.imparable){
                    if(MathUtils.random(0,100) < 25){
                        abilities.add(new Abilities(brick_strongs.get(i).position.x, brick_strongs.get(i).position.y));
                    }
                    this.brick_strongs.remove(i);
                }
            }
        }
    }
    ...
    private void gameAct(){
        if(bar.lives == 0){
            game.setScreen(new FinalScreen(game, "LEVEL 2", bricksBreaked));
            dispose();
        }
